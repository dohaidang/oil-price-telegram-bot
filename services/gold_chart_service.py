...
from services.gold_world_service import get_gold_world_history
from models.database import get_gold_vn_history
from utils.logger import setup_logger

logger = setup_logger("gold_chart_service")
...
async def generate_gold_world_chart(period: str = "1m") -> io.BytesIO | None:
    """Generate XAUUSD price chart."""
    yf_period = PERIOD_MAP.get(period, "1mo")
    period_label = PERIOD_LABELS.get(period, period)

    data = await get_gold_world_history("XAUUSD", yf_period)
    if not data:
        return None
    ...
    ax.plot(df["date"], df["close"], color=GOLD_COLOR, linewidth=2, label="XAUUSD", alpha=0.9)
    ax.fill_between(df["date"], df["close"], alpha=0.15, color=GOLD_COLOR)
    ...
    ax.annotate(
        ...
        fontsize=8, color=GOLD_COLOR, fontweight="bold", ha="center", va="bottom",
    )
    ax.annotate(
        f"${df['close'][min_idx]:.2f}",
        xy=(df["date"][min_idx], df["close"][min_idx]),
        fontsize=8, color=GOLD_COLOR, fontweight="bold", ha="center", va="top",
    )

    ax.set_title(
        f"🥇 Biểu Đồ Giá Vàng XAUUSD - {period_label}",
    ...
    fig.text(
        0.99, 0.01,
        f"Gold Price Bot • {datetime.now().strftime('%d/%m/%Y %H:%M')}",
    ...
    logger.info(f"Gold world chart generated - period: {period}")
    return buf


async def generate_gold_vn_chart(
    brand: str = "SJC",
    price_type: str = "sell",
    days: int = 30,
) -> io.BytesIO | None:
    """Generate gold VN price chart from DB history."""
    history = await get_gold_vn_history(brand, price_type, days)
    ...
    color = GOLD_COLOR if price_type == "sell" else GOLD_COLOR_2
    ...
    ax.set_title(
        f"🥇 Biểu Đồ Giá Vàng {brand} ({price_type.capitalize()}) - {days} ngày",
        fontsize=16, fontweight="bold", color=TEXT_COLOR, pad=15,
    )
    ax.set_xlabel("Ngày", fontsize=11, color=TEXT_COLOR)
    ...
    logger.info(f"Gold VN chart generated - {brand} {price_type} {days}d")
    return buf
